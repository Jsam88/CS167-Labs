...
object App {
  ...
  def main(args: Array[String]) {
    ...
    try {
      ...
      // TODO 1a: filter lines which do not start with "host\tlogname" from `inputRDD` PART 3
      val validLines: RDD[String] = inputRDD.filter(line => !line.startsWith("host\tlogname"))

      // TODO 1b: split each line by "\t" from `validLines` via `map` PART 3
      ...
      command match {
        case "count-all" =>
          ...
          // TODO 2: count total number of records in the file on `parsedLines` PART 4
        ...
        case "code-filter" =>
          ...
          val filteredLines: RDD[Array[String]] = parsedLines.filter(row => row(5) == responseCode)
...
//          val averages: RDD[(String, (Long, Long))] =
//            println(s"Average bytes per code for the file '$inputfile'")
//          println("Code,Avg(bytes)")
